...
\subsection{Variant configuration file}
...
\subsection{Parameter configuration files}
\label{sec:parameter_config_files}
...
We recommend to use the \rcode{AS} option to shorten the robot code a little bit.
...
Keys that are defined within \pcode{params['global']} in JSONP configuration files become \textbf{Robot Framework} variables. Therefore, the key names
must conform to \textbf{Robot Framework}'s naming conventions.

The naming convention is: Key names may only contain letters, digits and underscores, and must start with a letter.
Internally this is realized by the following regex pattern: \pcode{^\\p\{L\}[\\p\{L\}\\p\{Nd\}_]*$}.

\textbf{Example}

\begin{itemize}
   \item Valid key names are: \pcode{"abcParam"}, \pcode{"abcParam01"}, \pcode{"abc\_Param\_01"}, ...
   \item Invalid key names are: \pcode{"+param01"}, \pcode{"param\$01"}, \pcode{"abc\#Param"}, \pcode{"01abc\_Param"}, ...
\end{itemize}

\begin{boxwarning}{Caution}
The \textbf{Robot Framework} ignores underscores. Nevertheless, underscores are allowed inside variable names, but you must be aware of the consequences
when using them! The following code example demonstrates the effects.
\end{boxwarning}
...
\begin{boxwarning}{Impact}
Three \textbf{Robot Framework} variables with different names are created, but all variables have the same value. Therefore, underscores must not be used
...
\section{Version control interfaces}

The \textbf{RobotFramework AIO} supports a version control in two ways: \textit{implicitly} and \textit{explicitly}. An implicit version control is
executed automatically at the beginning of every test suite (during \rcode{testsuite_setup}).
A version expected by user and defined within the variant configuration file, is checked against the version of the
installed \textbf{RobotFramework AIO}. In detail this is explaind in \ref{sec:parameter_config_files}.

Version control can also be executed explicitly by the user. This version control can refer to any versions (not only to the versions of already
installed software such as \textbf{RobotFramework AIO} and \textbf{TestsuitesManagement}).

Additionally, it is possible to query the versions of \textbf{RobotFramework AIO} and \textbf{TestsuitesManagement} directly.

For this purpose, \textbf{TestsuitesManagement} provides two interfaces:

\begin{enumerate}
   \item \textbf{RobotFramework AIO} interface
   \item \textbf{Python} interface
\end{enumerate}

\subsection{RobotFramework AIO interface for version control}

Like usual, the \textbf{TestsuitesManagement} library needs to be imported in this way:

\begin{robotcode}
Library    RobotFramework_TestsuitesManagement    AS    tm
\end{robotcode}

\subsection{Explicit version control of the RobotFramework AIO}

The simplest implementation of a version control looks like this:
...
In this example, the specification is missing against which version (the \rcode{reference_version}) the \rcode{min_version}
and the \rcode{max_version} should be checked. This automatically leads to the version of the installed software being taken as reference.
There are two possibilities for this:

\begin{enumerate}
   \item The \textbf{TestsuitesManagement} is installed together with the \textbf{RobotFramework AIO} (thus is part of the entire
         \textbf{RobotFramework AIO} bundle).
         In this case, the version of the installed \textbf{RobotFramework AIO} is the \textit{reference} version.
   \item The \textbf{TestsuitesManagement} is installed standalone (for example separately via PyPi).
         In this case, the version of the installed \textbf{TestsuitesManagement} is the \textit{reference} version.
\end{enumerate}

We assume now that the \textbf{RobotFramework AIO} is installed with version \textbf{0.14.2.26}. This fits to the expectation of
mimimum version \textbf{0.1.0} and maximum version \textbf{0.20.0} from the example above. Therefore the result is:
...
Exactly this is what already happened during the \rcode{testsuite_setup}. Therefore this version check will not provide any new insights.
In the next example we check the version of the \textbf{TestsuitesManagement}.

\newpage

\subsection{Explicit version control of the TestsuitesManagement}

At first we get the version of the \textbf{TestsuitesManagement}. After this we set this version as reference:
...
log     === check_version result (TSM): ${result}    console=yes
\end{robotcode}

It can be seen that the expectation of minimum version \textbf{0.1.0} and maximum version \textbf{0.2.0} does not fit
to the current \textbf{TestsuitesManagement} version \textbf{0.9.0}. The version check and therefore also the test fails:
...
Whether the test result is \rlog{UNKNOWN} or \rlog{FAIL} depends on the type of the \textbf{RobotFramework AIO} installation.
Details can be found here: \href{https://robotframework-aio.org/}{robotframework-aio.org}.

\subsection{User defined version control}

The keyword \rcode{check_version} can be used for any version checks:

\begin{robotcode}
${result}=    tm.check_version    min_version=0.1.0    max_version=0.2.0    reference_version=1.2.3
\end{robotcode}

Versions that are not relevant for a version check, can be set to \rcode{None}. In this example only
the maximum version is checked against the reference:

\begin{robotcode}
${result}=    tm.check_version    min_version=${None}    max_version=0.2.0    reference_version=1.2.3
\end{robotcode}

If nothing is specified, the version check is skipped:

\begin{robotcode}
${result}=    tm.check_version    min_version=${None}    max_version=${None}    reference_version=${None}
\end{robotcode}
...
Because of a version check is not executed, the reference version doesn't matter.
...
The following formats are accepted for all version parameters:

\begin{itemize}
   \item \rcode{major.minor.patch}
   \item \rcode{major.minor}
   \item \rcode{major}
\end{itemize}
...
Every deviation cause a failed test:
...
\begin{robotlog}
ERROR - Version format error: maximum version '0.2.0.9' contains too many parts (expected is: major.minor.patch)
UNKNOWN - Execution will be aborted because of a critical version check issue
\end{robotlog}

\vspace{2ex}

It makes a difference, whether the reference version is defined or not!

\begin{itemize}
   \item \rcode{reference_version} is not defined:
\end{itemize}

Depending on the installed software, the TestsuitesManagement decides what is used as reference (either the \textbf{RobotFramework AIO} or
the \textbf{TestsuitesManagement}). This means, a name of the checked software is known. In this case the name is part of the logged output:
...
\begin{itemize}
   \item $\textbullet$ \rcode{reference_version} is defined:
\end{itemize}

In this case a corresponding application name that is assigned to this number, is not available. Therefore, the log does not contain
a name.
...
\begin{robotlog}
INFO - Version check is skipped because both 'min_version' and 'max_version' are set to None
\end{robotlog}

This is the same result as before, but without name (\textbf{RobotFramework AIO (extended)}).
...
\subsection{Python interface for version control}

The goal of this Python interface is to enable \textbf{RobotFramework AIO} users who work purely in Python, without using robot files,
to verify which bundle version is installed. The bundle version is either the version of the entire \textbf{RobotFramework AIO}
or, if installed standalone, the version of the \textbf{TestsuitesManagement}.
...
oVersion = CVersion()
result, reason = oVersion.verify_version(min_version, max_version)
\end{pythoncode}

The parameters \pcode{min_version} and  \pcode{max_version} define the expected range of the bundle version. This is verified
against the version of the currently installed bundle. The format of the version string is: \pcode{\\d\\.\\d\\.\\d}.

It is possible to set a version to \pcode{None} or to \pcode{\"\"}. The impact is that this version number (\textit{min}
or \textit{max}) is not part of the version check.

Two values are returned:

\begin{enumerate}
   \item \pcode{result}\\
        A boolean variable that is either \pcode{True} in case of the version check is passed or \pcode{False}
        in case of the version check fails.
   \item \pcode{reason}\\
        In case of the version check is passed, \pcode{reason} is \pcode{None}. In case the version check fails,
        \pcode{reason} contains a short token string indicating the reason for the failure.
\end{enumerate}

The method \pcode{verify_version} does not return a full error message, but a small token string only.
This enables users to formulate their own error messages.

These are the token strings together with their meanings:

\begin{itemize}
   \item \pcode{without_version_check} \\
         Version check not executed.

   \item \pcode{wrong_minmax} \\
         Mismatch of \emph{minimum version} and \emph{maximum version}: The \emph{minimum version} is younger
         than the \emph{maximum version}.

   \item \pcode{conflict_min} \\
         The test execution requires the \emph{minimum version}, but the installed version is older.

   \item \pcode{conflict_max} \\
         The test execution requires the \emph{maximum version}, but the installed version is younger.

   \item \pcode{internal_error} \\
         An internal error happened (any issue in JSON configuration file).
\end{itemize}

% --------------------------------------------------------------------------------------------------------------

% https://github.com/test-fullautomation/robotframework-testsuitesmanagement/issues/344
% https://github.com/test-fullautomation/robotframework-testsuitesmanagement/issues/352
% https://github.com/test-fullautomation/robotframework-testsuitesmanagement/issues/349



% We can tell the users:

    % The high-level method does all for you (logging of status messages, flow control by returns or exceptions). Feel free to use this method if you want to go the easy trail. It is possible to adapt the status messages, but it is not possible to adapt the return values and also not possible to avoid exceptions in case of critical errors.

    % If you want to have full control about the entire version check process, use the corresponding low-level method. This enables you to define own status messages in your own way and to define how you want to react on version check results. You can use the existing high-level method as a reference for implementation.
...
% Tabellarische Übersicht über die Prototypen der beteiligten Methodern und Keywörter


% "any versions" oder "any version"?
